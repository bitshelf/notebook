...
```sql
# 下面的例子就是列出含有 `hello`、`Hello`、`HeLLO` 等字段的文档：
​```dataview
list
where hello
​```

# 下面的例子就是列出含有 `Hello` 字段的文档：
​```dataview
list
where Hello
​```
...
# 下面的例子就是把文件的年份和月份加起来：
​```dataview
list file.ctime.year + file.ctime.month
​```
```
...
您可以通过“索引”链接来获取相应页面上的值。例如 `[[Link]].field` 将从页面链接获取相应的值。
...
都是自用的例子，只是能用的级别。下面依照自用的例子讲解下 dataview 和 dataviewjs 的一些用法。

- 问题
  - 项目、资源文件会随着时间越来越多，如果没有提前设计好一个合适的文件结构，届时肯定会有数据太多导致文档太多，难以及时寻找到自己想要的文档；又或者设计好了合适的文件存放结构，但是寻找文件还是需要自己手动点开文件所在的目录并找到文件，或者在一个主moc上手动添加每个子moc。

- 解决方法
  - 使用dataview就可以解决此类问题，实现文档的自动索引，方便我们去寻找文档。

当然，使用dataview的索引方式也有很多种，这里我只说下自己的方式。

**我的文档索引方案思路**：

1. 总体思路：设计好文档的存放结构，通过文档的路径进行索引
1. 具体方法：
   1. 先看[文件的存放方式](#文件存储方案)
      1. 一个项目或资源的**所有文档和附件**存放在一个**以项目或资源命名**的文件夹中；（**需要手动创建**文件夹）
      1. 创建一个**和项目或资源文件夹同名**的文档，该文档即该文件夹下的moc，可以用来索引该文档下的所有文档或未被链接的文档。（**需要手动创建**）
      1. **附件**存放在该文件夹下的assets文件夹中；（设置后无需手动创建，可在 obsidian 中设置，在添加附件时会自动创建该文件夹 并添加附件至该文件夹内）
   1. 配置MOC
      这里设置了两种moc：
      1. **主moc**：位于根目录，自动索引指定目录的子moc。
      1. **子moc**：位于具体的项目或资源文件夹中**和文件夹同名**的文档，自动索引其所在文件夹下的所有文档或未被其引用的文档。
1. 综上所述：
   1. 配置好主 moc 后，只需要按一定的方式1. 创建文件夹，2. 创建同名文件，即可在主 moc 中看到子 moc 的索引；
   1. 然后配置子 moc 即可实现索引当前文件夹下所有的其他文档或未被子 moc 链接的文档。

### 文件存储方案

我的文件存放方式大致如下：（其中**高亮标记的为文件夹，行内代码标记的为文档**）
...
- ==根目录==
  - ==项目==
    - ==Todo==
      - ==水质监测==（一个具体的项目文件夹）
        - ==.assets==（附件文件夹）
        - `水质监测.md`（这个是**和文件夹相同名称**的文档，也是该文件夹下所有文件的索引，即**子 moc**）
        - `文档1.md`（在编写文档时，可以设置obsidian新建笔记的存放位置为当前打开的笔记所在的文件夹）
        - `文档2.md`
        - ……
      - ……
    - ……
  - ==资源==
    - ==工具==（资源的分类）
      - ==markdown==（一个具体的资源文件夹）
        - ==.assets==
        - `markdown.md`（同样**和文件夹相同名称**，是该资源文件夹的**子 moc**）
        - `文档1.md`
        - `文档2.md`
        - ……
      - ……
    - ……
  - `主 moc 项目索引.md`（这个用来索引项目的**主 moc** 文件）
  - `主 moc 资源索引.md`（这个用来索引资源的**主 moc** 文件）

## dataview 示例：配置主、子 moc 实现项目、资源索引

### 配置主 moc

#### 预期功能

1. **索引子 moc**：实现自动索引指定路径（此处为 `项目/Todo/`）下的子moc；
1. **标签**：获得子moc的标签；
1. **创建日期**：获取子moc的创建日期并按降序排序。

#### 准备测试文件和[dataviewjs](#准备测试文件)中的差不多

#### 主 moc 代码

我这个文件存储索引方式通过dataview进行索引，这个逻辑有可能会出现问题，使用dataviewjs则不会。

当然也可以使用为子moc添加标签的方式来实现索引，但是个人认为任何多余的步骤或容易让人忘记或误操作的操作都不应该存在，我应该做的只有创建我要存放文档的文件夹、创建同名文档，然后编辑即可，不用再手动去为子moc添加标签什么的用来索引。

```sql
​```dataview
TABLE file.tags AS 标签, file.cday AS 创建日期
FROM "项目/Todo"
...
![image-20210816182638815](image-20210816182638815.png)

### 配置子 moc

#### 预期功能

获取当前文件夹下的所有任务并按文件的创建时间的降序排序

#### 准备测试文件和[dataviewjs](#准备测试文件)中的差不多

#### 子moc代码

其中的source可以配合template插件实现插入自动填写。
...
![image-20210816183432452](image-20210816183432452.png)

## dataviewjs 示例：配置主、子 moc 实现项目、资源索引

### 配置主 moc

配置主moc 索引，以项目中的Todo索引为例。

#### 预期功能

1. **索引子 moc**：实现自动索引指定路径（此处为 `项目/Todo/`）下的子moc；
1. **标签**：获得子moc所在文件夹下的所有文档的标签并不重复地显示；
1. **任务**：获取子moc所在文件夹下的所有文档的任务并计算进度。
   1. 并且要求任务列表有多个层级时，当父任务完成时，则无论父任务下的子任务是否完成，都认为其已全部完成

预期效果如下：

![image-20210816163355182](image-20210816163355182.png)

#### 准备测试文件

##### 1. 创建文件夹及文件用于测试

文件夹以项目/资源名称命名；子moc和文件夹同名。（即按照[我自己的文件存储方式](#文件存储方案)创建相应文件）

下面创建两个项目文件夹，并未每个文件夹创建一个同名笔记（即子moc）。

![image-20210816164222137](image-20210816164222137.png)
...
1. 首先获取子moc文件，使用dataview中的where即可；
1. 然后遍历每个子moc所在文件夹下的所有文档，获取处理其标签、任务信息并存储至子moc文件对象中；
1. 使用 `dv.table()` 展示处理好的文档。
...
const Path = "项目/Todo/";	// 需要手动指定该目录

//========================================================================================
// 获取子 moc 文件对象
var mocFiles = dv.pages().file	//indexOf、endsWith自己百度一哈，js有很多函数，自行百度即可
	.where(b => b.path.indexOf(Path)!=-1 && b.path.endsWith(b.name + '/' + b.name + '.md'))

//========================================================================================
// 计算任务进度用到的全局变量
var allTask = 0;		//每个子moc的任务总数
var completedTask = 0;	//每个子moc的完成任务总数
/*处理任务进度的自定义函数
*	用于遍历任务计算任务总数和任务完成数，且若父任务完成，则认为其子任务全部完成。
*	参数：
*		tasks：DataArray中的tasks对象（可以使用console.log()输出tasks的属性信息到控制台(Ctrl+Shift+i)查看）
*		superTaskFullyCompleted: 父任务是否完成？完成的话传true，否则传false
*/
function taskSolve(tasks, superTaskFullyCompleted){
	for(var task of tasks){
		allTask++;							//无论是否完成，任务总数+1
		if(!superTaskFullyCompleted && !task.completed){	//如果父任务和当前任务均未完成，则说明任务确实未完成
			taskSolve(task.subtasks, false)	//调用自身
		}
		else{												//否则任务完成
			completedTask++;				//任务完成总数+1
			taskSolve(task.subtasks, true)	//调用自身
		}
	}
}
...
for(var mocFile of mocFiles){
	...
	// 处理标签
	//=========================================================
	// 获取子moc文件所在文件夹下的所有标签，并将标签无重复地显示为 “#标签1 #标签2 ……” 的形式
	var allTags = new Array();	//创建一个新的数组用于存储查询得到的标签
	for(var file of dv.pages().file.where(b => b.path.match(mocFile.folder))){	//遍历子moc所在文件夹下的页面
		for(var tag of file.tags){		//遍历每个页面的标签
			if(allTags.indexOf(tag)==-1){	//如果标签之前未存储过，则将其存储起来
				allTags.push(tag)
			}
		}
	}
	// 把存储的标签列表拼接成字符串的形式
	mocFile["myTags"] = '';	//调用的是mocFile对象的myTags键，对该值进行操作，值会存储在mocFile["myTags"]中
	...
	//=========================================================
	allTask = 0;		//任务总数清零
	completedTask = 0;	//完成任务总数清零
	taskSolve(dv.pages().file.tasks.where(t => t.path.match(mocFile.folder)), false);	//自定义任务处理函数，用于遍历任务
	mocFile["jindu"] = '';
	if(allTask){
		mocFile["jindu"] = '任务完成进度：**' + (completedTask/allTask * 100).toFixed(1) + '%**';	//计算进度
	}
}
...
dv.table(
	['文件', '标签', '任务'],			  //表头
	mocFiles					//要展示的DataArray
	.sort(b => -b.ctime)				//排序，按照子moc的创建时间降序排序
	.map(b => [b.link, b.myTags, b.jindu])	//根据表头确定要显示的内容
)
​```
```
...
![image-20210816172125944](image-20210816172125944.png)

### 配置子 moc

#### 预期功能

1. **任务**：获取子moc所在文件夹下所有文档的任务视图，并同主moc中一样的方法计算任务进度；
1. **未被子moc链接的文档**：获取子moc所在文件夹下未被子moc以 `[[文件名.md]]` 形式链接的文档。

#### 准备测试文件

继续使用[配置主moc时准备的配置文件](#准备测试文件)就行了，主要使用水质监测项目进行测试。
...
//调用函数计算任务进度
taskSolve(dv.pages().file.tasks.where(t => t.path.match(dv.current().file.folder)));
//如果有任务则计算并输出任务进度
if(allTask){
	dv.paragraph('任务完成进度：**' + (completedTask/allTask * 100).toFixed(1) + '%**')		//渲染一段文字
}
...
dv.taskList(
	dv.pages().file.tasks
	.where(t => t.path.match(dv.current().file.folder))	//筛选仅在子moc所在文件夹下的文档
	.sort(t => -t.ctime)		//按任务的创建时间降序排列
)
...
成功显示当前文件夹下所有的任务及完成进度，$2/3≈66.7\%$。

![image-20210816175931101](image-20210816175931101.png)
...
//========================================================================================
// 初步筛选以获取子moc所在文件夹下除了子moc以外的所有文档
var currentFolderFilesExceptThis = dv.pages().file
	.where(b => b.path.indexOf(dv.current().file.folder)!=-1 && b.path!=dv.current().file.path);

//========================================================================================
// 进一步筛选，获取未被子moc链接的文档链接
var notLinkedPages = new Array();	//创建新数组用于存放未被链接的文件
var linked;	//标志位，用于判断文档是否被链接
for(var file of currentFolderFilesExceptThis){		//遍历筛选过的文档
	linked = 0;			//链接标志位清零
	for(var outlink of dv.current().file.outlinks){	//遍历当前文档的外链
		if(outlink.path==file.path){		//如果外链中的链接路径等于文档路径，说明该文档被子moc链接了
			linked = 1;	//链接标志位置位
			break;		//跳出循环
		}
	}
	if(!linked){		//如果未被链接，则将其储存至数组
		notLinkedPages.push(file)
		//notLinkedPages.push(file.link)		//前面说过，list也接受普通数组，所以也可以直接存储文档的链接然后展示
	}
}

//========================================================================================
// 列出未被子moc链接的文档
dv.list(dv.array(notLinkedPages).link);	// 将普通数组转换为DataArray类型，然后展示
//dv.list(notLinkedPages);	// 若上文是直接存储文档的链接，则不用进行数组转换，直接显示即可
...
![image-20210816175718050](image-20210816175718050.png)